...
public class VacuumView extends EmptyEnvironmentView {
	...
	private Hashtable<Agente, Accion> lastActions = new Hashtable<Agente, Accion>();
	...
	public void agentActed(Agente agent, Accion action,
		...
		String prefix = "";
		if (env.getAgentes().size() > 1)
			prefix = "A" + env.getAgentes().indexOf(agent) + ": ";
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < locations.size(); i++) {
			...
			Agente agent = getAgent(location);
			if (isDirty(location)) {
			...
			if (agent != null) {
				Accion action = lastActions.get(agent);
				g2.setColor(Color.RED);
				if (action == null || !((AccionDinamica) action).getAttribute("name").equals("Suck")) 
	 ...
	 * */
	protected Agente getAgent(Object location) {
		VacuumEnvironment e = getVacuumEnv();
		for (Agente a : e.getAgentes())
