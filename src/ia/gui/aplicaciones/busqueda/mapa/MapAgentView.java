...
import ia.gui.framework.EmptyEnvironmentView;
...
public class MapAgentView extends EmptyEnvironmentView {
	...
	private final Hashtable<Agente,List<String>> agentTracks = new Hashtable<Agente,List<String>>();
	...
	public List<String> getTrack(Agente agent) {
	 ...
	 */
	@Override
	public void agentActed(Agente agent, Accion command, EstadoEntorno state) {
		MapEnvironment mEnv = getMapEnv();
		String msg = "";
		if (mEnv.getAgentes().size() > 1)
			msg = "A" + mEnv.getAgentes().indexOf(agent) + ": ";
	...
	private void paintTrack(java.awt.Graphics2D g2, Agente a) {
		Map map = getMapEnv().getMap();
		Point2D lastPt = null;
	...
	protected void paintLoc(java.awt.Graphics2D g2, String loc) {
		...
		if (pt != null) {
			...
			if (!env.getAgentes().isEmpty())
				...
				track = getTrack(env.getAgentes().get(0));
