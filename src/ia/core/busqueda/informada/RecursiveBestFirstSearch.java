...
import ia.core.busqueda.framework.ExpansorDeNodo;
 ...
 */
public class RecursiveBestFirstSearch extends ExpansorDeNodo implements Busqueda {

	private final FuncionEvaluacion evaluationFunction;
	...
	public RecursiveBestFirstSearch(FuncionEvaluacion ef) {
	...
	public List<Accion> search(Problema p) throws Exception {
		...
		Nodo n = new Nodo(p.getInitialState());
		SearchResult sr = rbfs(p, n, evaluationFunction.f(n), INFINITY, 0);
		if (sr.getOutcome() == SearchResult.SearchOutcome.SOLUTION_FOUND) {
			Nodo s = sr.getSolution();
			actions = UtilidadesDeBusqueda.actionsFromNodes(s.getPathFromRoot());
	...
	// a new f-cost limit
	private SearchResult rbfs(Problema p, Nodo n, double node_f, double fLimit,
			int recursiveDepth) {

		setMaxRecursiveDepth(recursiveDepth);

		// if problem.GOAL-TEST(node.STATE) then return SOLUTION(node)
		if (UtilidadesDeBusqueda.isGoalState(p, n)) {
		...
		// for each action in problem.ACTION(node.STATE) do
		// add CHILD-NODE(problem, node, action) into successors
		List<Nodo> successors = expandNode(n, p);
		// if successors is empty then return failure, infinity
...
class SearchResult {
	...
	private Nodo solution;

	private SearchOutcome outcome;

	private final Double fCostLimit;

	public SearchResult(Nodo solution, Double fCostLimit) {
		if (null == solution) {
