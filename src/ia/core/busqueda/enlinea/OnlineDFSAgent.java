 ...
 * 
 */
public class OnlineDFSAgent extends AgenteAbstracto {
	...
	// untried, a table that lists, for each state, the actions not yet tried
	private final Map<Object, List<Accion>> untried = new HashMap<Object, List<Accion>>();
	...
	public OnlineDFSAgent(OnlineSearchProblem problem,
			FuncionPercepcionAEstado ptsFunction) {
		setProblem(problem);
		...
		if (goalTest(sDelta)) {
			...
			if (untried.get(sDelta).isEmpty()) {
				...
				if (unbacktracked.get(sDelta).isEmpty()) {
					...
					for (Pair<Object, Accion> sa : result.keySet()) {
	...
	private List<Accion> actions(Object state) {
		return new ArrayList<Accion>(problem.getActionsFunction()
				.actions(state));
	}
}
