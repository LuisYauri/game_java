...
import ia.core.util.SetOps;
import ia.core.util.estructuradedatos.FIFOQueue;
import ia.core.util.estructuradedatos.Queue;
 ...
 */
public class HybridWumpusAgent extends AgenteAbstracto {
	...
	private int t = 0;
	// plan, an action sequence, initially empty
	private Queue<Accion> plan = new FIFOQueue<Accion>();
	...
	/**
	 * Returns a sequence of actions using A* Busqueda.
	 * 
	 * @param current
	 ...
	 */
	public List<Accion> planRoute(AgentPosition current, Set<Room> goals,
			Set<Room> allowed) {
		...
		PruebaDeMeta goalTest = new PruebaDeMeta() {
	 ...
	 */
	public List<Accion> planShot(AgentPosition current,
		...
		List<Accion> actions = planRoute(current, shootingPositionsArray,
				allowed);
