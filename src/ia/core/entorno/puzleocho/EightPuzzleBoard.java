...
public class EightPuzzleBoard {
	...
	public static Accion LEFT = new AccionDinamica("Left");

	public static Accion RIGHT = new AccionDinamica("Right");

	public static Accion UP = new AccionDinamica("Up");

	public static Accion DOWN = new AccionDinamica("Down");
	...
	public int getValueAt(UbicacionXY loc) {
		return getValueAt(loc.getCoordenadaX(), loc.getCoordenadaY());
	}

	public UbicacionXY getLocationOf(int val) {
		int absPos = getPositionOf(val);
		return new UbicacionXY(getXCoord(absPos), getYCoord(absPos));
	}
	...
	public List<UbicacionXY> getPositions() {
		ArrayList<UbicacionXY> retVal = new ArrayList<UbicacionXY>();
		for (int i = 0; i < 9; i++) {
			int absPos = getPositionOf(i);
			UbicacionXY loc = new UbicacionXY(getXCoord(absPos),
					getYCoord(absPos));
			retVal.add(loc);
	...
	public void setBoard(List<UbicacionXY> locs) {
		int count = 0;
		for (int i = 0; i < locs.size(); i++) {
			UbicacionXY loc = locs.get(i);
			this.setValue(loc.getCoordenadaX(), loc.getCoordenadaY(), count);
			count = count + 1;
		}
	}

	public boolean canMoveGap(Accion where) {
		boolean retVal = true;
