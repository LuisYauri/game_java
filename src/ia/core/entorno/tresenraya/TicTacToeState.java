...
public class TicTacToeState implements Cloneable {
	...
	public void mark(UbicacionXY action) {
		mark(action.getCoordenadaX(), action.getCoordenadaY());
	}
	...
	public List<UbicacionXY> getUnMarkedPositions() {
		List<UbicacionXY> result = new ArrayList<UbicacionXY>();
		for (int col = 0; col < 3; col++) {
			for (int row = 0; row < 3; row++) {
				if (isEmpty(col, row)) {
					result.add(new UbicacionXY(col, row));
				}
			}
		}
		return result;
